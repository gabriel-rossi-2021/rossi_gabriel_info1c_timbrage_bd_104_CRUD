    ...
    try:
        ...
        if request.method == "GET":
            with MaBaseDeDonnee().connexion_bd.cursor() as mc_afficher:
                strsql_genres_afficher = """SELECT id_genre, nom_famille FROM t_genre ORDER BY id_genre ASC"""
            ...
            for i in data_genres:
                genre_val_list_dropdown.append(i['nom_famille'])

            # Aussi possible d'avoir un id numérique et un texte en correspondance
            # genre_val_list_dropdown = [(i["id_genre"], i["nom_famille"]) for i in data_genres]
